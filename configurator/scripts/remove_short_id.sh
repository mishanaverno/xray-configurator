...
if ! updated_value="$(jq -cn --argjson current "$current_value" --arg short_id "$short_id" '$current | map(select(. != "" and ascii_downcase != $short_id)) | unique | if length == 0 then [""] else . end')" ; then
