...
Configurator-контейнер собирает и запускает Xray-конфигурацию из встроенного шаблона, управляет SNI-кандидатами, geodata-файлами, client routing и shortId для пользователей.
...
Nginx использует единый stream proxy на `:443` и прокидывает трафик в `127.0.0.1:8443` с proxy protocol. Preset должен быть совместим с таким входом.

Preset `reality_xhttp_slave` также может управлять slave через SSH. Для этого используются локальные endpoints:
...
Они выполняют на slave-сервере локальные configurator API-команды через SSH.
...
Встроенный дефолтный шаблон хранится в `configurator/preset` и копируется в образ как:
...
Копирование происходит только если файла в volume еще нет. Контейнер не переключает preset'ы на runtime: чтобы изменить встроенный шаблон, нужно пересобрать образ с другим содержимым `configurator/preset`.
...
Часть файлов опциональна: если нет `client_routing.json`, `link.txt` или `sni_list`, соответствующие endpoint'ы вернут понятную ошибку.

Nginx-конфиг лежит отдельно:

```text
nginx/reality.conf
```

`entrypoint.sh` всегда использует `nginx/reality.conf`.
...
3. Копирует встроенные templates в volume.
...
6. Генерирует `/usr/share/xray/link.txt`, если шаблон содержит `templates/link.txt`.

Рабочие `config.json` и `link.txt` заменяются только после успешной генерации во временные файлы. Если шаблон не содержит `link.txt`, старый `/usr/share/xray/link.txt` удаляется.
...
Проверяет `XRAY_REALITY`, затем генерирует config и запускает Xray.
...
Возвращает содержимое `link.txt`, если текущий шаблон генерирует клиентскую ссылку.
