...
: "${TLS_CERT_FILE:?TLS_CERT_FILE is not set}"
: "${TLS_KEY_FILE:?TLS_KEY_FILE is not set}"
...
say "[ensure_xhttp_tls.sh] Generating self-signed TLS certificate for $HOST..."
...
chmod 664 "$TLS_CERT_FILE" 2>>"$LOG_FILE" || fail "Failed to chmod certificate"
chmod 660 "$TLS_KEY_FILE" 2>>"$LOG_FILE" || fail "Failed to chmod private key"
