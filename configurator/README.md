...
По умолчанию используется встроенный preset:
...
Для серверных preset'ов часть файлов может отсутствовать. Например, `xhttp_slave` не содержит `client_routing.json`, `link.txt` и `sni_list`, потому что не выдает клиентские ссылки и не использует Reality/SNI.
