...
if [[ ! -f "$TEMPLATES_DIR/$SECRETS_FILE" ]]; then
  echo "[entrypoint] Generating fresh UUID and x25519..."
  ...
  echo "[entrypoint] Secrets generated: UUID=$uuid PUB_KEY=$pass PRIV_KEY=$priv"
...
EOF
  ...
  echo "[entrypoint] secrets.env already exists"
fi
